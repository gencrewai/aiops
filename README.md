...
> 그 밖에 사용 가능한 컴포넌트: `project-name`, `context-remaining`, `used-tokens`, `codex-version`, `task-progress`, `fast-mode`, `thread-title` 등. `codex /statusline`(인터랙티브) 또는 `~/.codex/config.toml` 직접 편집으로 조합을 바꿀 수 있습니다.
